...
export class Init1689178287465 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`view_roles_role\` (\`viewId\` int NOT NULL, \`roleId\` int NOT NULL, INDEX \`IDX_fdc117537408710e3a39a6e946\` (\`viewId\`), INDEX \`IDX_13810c6598c5585d739a531c7a\` (\`roleId\`), PRIMARY KEY (\`viewId\`, \`roleId\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`view_roles_role\` ADD CONSTRAINT \`FK_fdc117537408710e3a39a6e9460\` FOREIGN KEY (\`viewId\`) REFERENCES \`view\`(\`id\`) ON DELETE CASCADE ON UPDATE CASCADE`);
        await queryRunner.query(`ALTER TABLE \`view_roles_role\` ADD CONSTRAINT \`FK_13810c6598c5585d739a531c7a6\` FOREIGN KEY (\`roleId\`) REFERENCES \`role\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);

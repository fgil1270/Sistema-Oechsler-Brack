...
export class DocumentEmployeeService {
    ...
    constructor(
        @InjectRepository(DocumentEmployee) private documentEmployeeRepository: Repository<DocumentEmployee>,
        @Inject(forwardRef(() => EmployeesService) ) private employeesService: EmployeesService,
    ...
    async create(data: CreateDocumentEmployeeDto){
        ...
        const document = await this.documentService.findById(Number(data.documentId));
        ...
        const newFile = await this.documentEmployeeRepository.save(createDocumentEmployee);
        return newFile;
    }
    ...
    async findByEmployeeAndDocumentNameAndDocument(employeeId: number, name: string, documentId: number){
        const file = await this.documentEmployeeRepository.findOne({
            relations: {
                employee: true,
                document: true
    ...
    async update(id: number){
        ...
        file.updated_at = new Date(new Date().toLocaleString("en-US", { timeZone: "America/Mexico_City" }));

...
export class DocumentClasificationService {
    ...
    async findByName(name: string){
        const documentClasification = await this.documentClasificationRepository.findOne({ where: { name } });
        return documentClasification;
    }
    ...
    async uploadFile(files: Express.Multer.File[]){
        ...
        try {
            for (let index = 0; index < files.length; index++) {
                ...
                let newDocument: any;
                let newfile: any;
                const employee = await this.employeesService.findByEmployeeNumber([name[0]]);
                ...
                newDocumentClasification = documentClasification ? documentClasification : await this.create({name: name[1]}) ;

                //si no existe el documento
                //crearlo 
                let nameDocument = name[2].split('.');
                newDocument = document ? document : await this.documentService.create({name: nameDocument[0], required: true, documentClasificationId: newDocumentClasification.id});

    
                const path = join(__dirname, `../../../documents/empleados/${name[0]}/${newDocumentClasification.name}/${newDocument.name}`);
                const filepath = join(path, name[2]);
    
                //si el archivo ya existe
                //se actualiza
                const file = await this.documentEmployeeService.findByEmployeeAndDocumentNameAndDocument(employee.emps[0].id, name[2], Number(newDocument.id));
                if(file){
                    const updateFile = await this.documentEmployeeService.update(file.id);
                }
                let url = `documents/empleados/${name[0]}/${newDocumentClasification.name}`;
                newfile = file ? file : this.documentEmployeeService.create({name: name[2], employeeId: employee.emps[0].id, documentId: newDocument.id, route: url});
    ...
    async getFilePath(idFile: number){
        ...
        return {
            path: join(__dirname, `../../../${file.route}/${file.document.name}`, file.name),
            fileName: file.name
        }

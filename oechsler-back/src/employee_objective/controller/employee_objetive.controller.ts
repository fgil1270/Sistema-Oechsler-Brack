...
export class EmployeeObjetiveController {
    ...
    status = {code: 200, message: 'OK', error: false, data: {}, status: 'success'};
    ...
    async createDefinitionObjectiveAnnualPartial(
        ...
        return this.employeeObjetiveService.createDefinitionObjectiveAnnual(currData, user);
    }
    ...
    @ApiOperation({ summary: 'Obtiene todos los objetivos de un empleado por id de objetivo' })
    @Get(':id')
    async findObjectiveEmployee(
    ...
    async downloadPdf(
        ...
        try {
            const pdfStream = await this.employeeObjetiveService.downloadPdf(currData);
            pdfStream.pipe(res);
    ...
    @ApiOperation({ summary: 'Obtiene todos los objetivos de un empleado por id de empleado' })
    ...
    async updateObjective( 
        ...
        switch (currData.action) {
            case 'updateCommentEdit':
                ...
                const result1 = await this.employeeObjetiveService.updateDefinitionObjective(currData.id, dataDefinition, user);
            ...
            case 'updateObjective':
                ...
                const result2 = await this.employeeObjetiveService.updateObjective(dataObjecyive.id, dataObjecyive, user);
            ...
            case 'updateDnc':
                ...
                const result3 = await this.employeeObjetiveService.updateDnc(dataDnc.id, dataDnc, user);
            ...
            case 'updateDncManual':
                const dataDncManual: UpdateDncCourseManualDto = currData.dncCourseManual;

                const result4 = await this.employeeObjetiveService.updateDncManual(dataDncManual.id, dataDncManual, user);
    ...
    @ApiOperation({ summary: 'Se Elimina informacion de la asignacion de objetivos' })
    @Delete('partial')
    async deleteDetail(
        @Query() currData: any,
        @CurrentUser() user: any

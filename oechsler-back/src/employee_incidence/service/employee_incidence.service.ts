...
export class EmployeeIncidenceService {
  ...
  async getReportPendingIncidence() {
    ...
    lideres.push(...jefeTurno.map(jefe => ({ idLider: jefe.id, email: '', empleados: [] })));
  ...
  async getReportPendingIncidence48() {
    ...
    for (let i = 0; i < idsLider.length; i++) {
      ...
      try {
        incidencias = await this.employeeIncidenceRepository.find({
          ...
          where: {
            ...
            status: 'Pendiente',
            created_at: LessThan(new Date(format(diaAyer, 'yyyy-MM-dd'))),
          },
          order: {
            employee: {
              id: 'ASC',
      ...
      let mailData = {
        ...
        empleados: jefeLider.name + ' ' + jefeLider.paternal_surname + ' ' + jefeLider.maternal_surname,
      }
      ...
      //envio de correo
      if (0 > 0) { //|| mailData.totalTimeCorrection > 0) {
